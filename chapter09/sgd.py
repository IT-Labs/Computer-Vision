...
def sigmoid_deriv(x):
    ...
    # that he input `x` has already been passed through the sigmoid activation function
    return x * (1 - x)
...
for epoch in np.arange(0, args["epochs"]):
    ...
    for (batchX, batchY) in next_batch(trainX, trainY, args["batch_size"]):
        ...
        # now that we gave our predictions, we need to determine the `error`, which is the
        # difference between our predictions and the true values
        error = preds - batchY
        epochLos.append(np.sum(error ** 2))

        # the gradient descent update is the dot product beween our (1) current batch
        # and (2) the error of the sigmoid derivative our predictions
        d = error * sigmoid_deriv(preds)
        gradient = batchX.T.dot(d)

        # in the update stage, all we need to do is "nudge" the weight matrix in the negative
        # direction of the gradient (hence the term "gradient descent") by taking a s small tep
        # towards a set of "more optimal" parameters
        W += -args["alpha"] * gradient

    # update our los history by taking the average loss across all batches
    loss = np.average(epochLos)
    losses.append(loss)
